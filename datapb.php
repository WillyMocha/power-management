...
?>
  ...
  <body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1> Data Pascabayar</h1>
        </div>
        <div class="button-tambah">
        <a href="proses/form_tambah_pb.php"><input type="submit" name="login" value="Tambah Pascabayar" class="btn btn-block" style="background-color:gray;width:200px;font-family:calibri light"></a>
        </div>
      </div>
      <div class="row">
        <div class="col-md-12">
          <div class="tile">
                ...
                <?php 
                  if(isset($_SESSION['alogin'])){
                    try{
                      ...
                      $sql = "SELECT id_pb, jumlah, kwatt, hari FROM pascabayar WHERE username='$username'";
                  ...
                  if($query->rowCount() > 0)
                  {
                    foreach($results as $result)
                      {       ?>  
                        <tr style="text-align:center">
                          <td><?php echo htmlentities($result->id_pb);?></td>
            ...
            <?php
                $kwatt = $result->kwatt;
                $hari = $result->hari;
                $daily_kwatt = $kwatt/$hari;
                echo("Daya Perangkat/Hari: ");
                echo($daily_kwatt);
                echo(" kWh");
            ?>
          </div>
